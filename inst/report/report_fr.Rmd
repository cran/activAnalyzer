...
if (df1_means[["total_steps"]] > 18000) {g_steps <- g_steps + annotate("text", label = "Le score est \nau-delà des limites!", x = 0.25, y = 9000, size = 7, color = "red", fontface = "bold")}
...
if (df1_means[["minutes_MVPA"]] > 100) {g_mvpa <- g_mvpa + annotate("text", label = "Le score est \nau-delà des limites!", x = 50, y = 0.5, size = 7, color = "red", fontface = "bold")}

# SED
g_sed <- create_fig_sed(score = df1_means[["minutes_SED"]], "fr") + theme(plot.margin = margin(0, 1, 0, 1, "cm"))
if (df1_means[["minutes_SED"]] > 15*60) {g_sed <- g_sed + annotate("text", label = "Le score est \nau-delà des limites!", x = 7.5, y = 2, size = 7, color = "red", fontface = "bold")}

# MVPA/SED ratio
g_ratio <- create_fig_ratio_mvpa_sed(score = df1_means[["ratio_mvpa_sed"]], "fr") + theme(plot.margin = margin(0, 1, 0, 1, "cm"))
if (df1_means[["ratio_mvpa_sed"]] > 0.4) {g_ratio <- g_ratio + annotate("text", label = "Le score est \nau-delà des limites!", x = 0.2, y = 0.8, size = 7, color = "red", fontface = "bold")}

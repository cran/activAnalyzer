...
# Results and export
In the app, the results by day and those summarized (means and medians) using valid days are shown in tables and figures. Four different buttons allow the user to generate a report where all the inputs of the app are recorded, as well as the results. Two first buttons allow getting an .html report (long format) in either english or french language, with all the tables and figures the app has generated. The two other buttons allow getting a .pdf report (short format) in either english or french language, with similar information to the long version but withtout the tabular views of the results for each day. In all reports, some comments are provided to help positioning the patient in relation to normative values or guidelines. The user also can click on specific buttons to export to .csv files either the marked dataset, the results by day, the mean results based on valid days, or the median results based on valid days. Finally, a last button allows to lead the user towards panels to fullfill the  *Daily- and Clinical
